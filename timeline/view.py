...
from .menu_builder import MenuBuilderDialog
...
class TimelineView(QtWidgets.QWidget):
    ...
    def contextMenuEvent(self, e: QtGui.QContextMenuEvent):
        ...
        paste_track = tr_under if tr_under else (hit[0] if hit else None)
        if paste_track is None and self._clipboard:
            paste_track = self._clipboard.get("track")
        actCopy = menu.addAction("Copy")
        actCopy.setEnabled(hit is not None)
        actPaste = menu.addAction("Paste")
        actPaste.setEnabled(self._clipboard is not None and paste_track is not None)

        menu.addSeparator()
        actUndo = menu.addAction("Undo")
        actUndo.setEnabled(bool(self._undo_stack))
        actRedo = menu.addAction("Redo")
        actRedo.setEnabled(bool(self._redo_stack))
        ...
        if hit:
            ...
            actCut = menu.addAction("Cut Here")
        ...
        if action == actUndo:
            ...
            self._redo()
        elif action == actCopy and hit:
            tr, k, _ = hit
            self._clipboard = {
                "track": tr,
                "data": copy.deepcopy(k.data or {}),
            }
        elif action == actPaste and paste_track and self._clipboard:
            self._push_undo()
            payload = self._clipboard
            data_copy = copy.deepcopy(payload.get("data", {}))
            kf = Keyframe(self.x_to_time(pos.x()), paste_track, data_copy)
            self.model.add_kf(paste_track, kf, snap=True)
            self._select_only(paste_track, kf.id)
        elif action == actDel:
            self._push_undo()
            self._delete_sel()
            ...
            redo_backup = list(self._redo_stack)
            self._push_undo()
            if not self._add_menu_at(self.x_to_time(pos.x())):
                if self._undo_stack:
                    self._undo_stack.pop()
                self._redo_stack = redo_backup
        elif action == actAddFx:
            self._push_undo()
            self._add_fx_at(self.x_to_time(pos.x()))
        elif action == actCut and hit:
            tr, k, _ = hit
            cut_time = self.x_to_time(pos.x())
            if cut_time > k.t + 1e-6:
                self._push_undo()
                new_dur = max(TICK_SEC, cut_time - k.t)
                k.data = dict(k.data or {})
                k.data["duration"] = float(new_dur)
                self.model.touch(tr, recompute_duration=True)
                self.update()
    ...
    def _add_menu_at(self, t: float) -> bool:
        t = self._snap(t, "MENU", ignore_id=None)
        opener = getattr(self.window(), "open_script_asset", None)
        dlg = MenuBuilderDialog(self.model, parent=self, script_opener=opener)
        ...
        if not payload.get("options"):
            payload["options"] = [{"text": "Option", "target": ""}]
        kf = Keyframe(t, "MENU", payload)
    ...
    def _restore_snapshot(self, snap: str):
        project = str(self.model.project_file) if self.model.project_file else None
        self.model.load_json(snap, project_file=project)
        self._selected = None
        self.selChanged.emit("", -1)
        self.update()
